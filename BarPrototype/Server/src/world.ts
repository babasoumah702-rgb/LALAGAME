...
import {buildProfile,selectPack,DEFAULT_PACK} from './identity.js';
...
export const zone=(s:Scenario,p:Point)=>p.area&&p.area!=='bar'?{id:p.area,name:({corridor:'走廊',stairs:'楼梯',rooftop:'屋顶'} as Record<string,string>)[p.area]??p.area,x:p.x,z:p.z,y:p.y,area:p.area,radius:3,privacy:.75,capacity:8}:s.locations.reduce((best,l)=>distance(p,l)/l.radius<distance(p,best)/best.radius?l:best);
...
export function createWorld(s:Scenario,nav:Navigator,options:{playerId:string;role?:string;entryIntent?:string;style?:string;seed?:number;online?:boolean;choices?:Record<string,string>;entryContext?:string}):World {
  const role=s.roles.find(r=>r.id===options.role)??s.roles.find(r=>r.id==='passerby')!;
  const seed=options.seed??821;
  const contextProfile=buildProfile(options.choices,options.entryContext);
  const {packId}=selectPack(contextProfile,s.identityPacks,seed);
  ...
  return {version:1,scenarioId:s.id,id:randomUUID(),playerId:options.playerId,role:role.id,entryIntent:options.entryIntent??'observe_only',style:options.style??'natural',seed,rng:seed,elapsed:0,night:1,status:'playing',paused:false,sequence:0,actors,events:[],jobs:[],beatIds:[],commandIds:[],flags:{},moves:{},cooldowns:{},initialRelations:Object.fromEntries(actors.filter(a=>a.id!=='USER').map(a=>[a.id,structuredClone(a.relations.USER)])),identityPack:packId,contextProfile,calls:0,tokens:0,modelMode:options.online?'online':'offline',modelReason:options.online?'联网推理':'规则模式 · 无模型调用',createdAt:now,updatedAt:now};
}
